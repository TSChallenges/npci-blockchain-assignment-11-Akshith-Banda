...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating tata motors Identities"
    ...
    infoln "Creating chase Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-chase.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
